...
local timeSyncedWithMachine = GetConvar('menu_sync_time_to_machine_time', 'false') == 'true'
local timeFrozen = GetConvar('menu_freeze_time', 'false') == 'true'
local currentHour = tonumber(GetConvar('menu_current_hour', '7'))
local currentMinute = tonumber(GetConvar('menu_current_minute', '0'))
...
local blackout = GetConvar('menu_enable_blackout', 'false') == 'true'
local snowEffects = GetConvar('menu_enable_snow_effects', 'false') == 'true'
...
local weatherChangeTime = tonumber(GetConvar('menu_weather_change_time', '5'))
...
lib.registerMenu({
    id = 'menu_world_related_options',
    title = 'World Related Options',
    position = 'top-right',
    onClose = function(keyPressed)
        CloseMenu(false, keyPressed, 'menu_main')
    end,
    onSelected = function(selected)
        MenuIndexes['menu_world_related_options'] = selected
    end,
    options = {
        {label = 'Time Options', args = {'menu_time_options'}},
        {label = 'Weather Options', args = {'menu_weather_options'}}
    }
}, function(_, _, args)
...
lib.registerMenu({
    id = 'menu_time_options',
    title = 'Time Options',
    position = 'top-right',
    onClose = function(keyPressed)
        CloseMenu(false, keyPressed, 'menu_world_related_options')
    end,
    onSelected = function(selected)
        MenuIndexes['menu_time_options'] = selected
    end,
    onCheck = function(_, checked, args)
        if args[1] == 'show_time' then
            showTimeOnScreen = checked
            lib.setMenuOptions('menu_time_options', {label = 'Show Time On Screen', args = {'show_time'}, checked = showTimeOnScreen, close = false}, 3)
        end
    end,
    options = {
    ...
    timeSyncedWithMachine = GetConvar('menu_sync_time_to_machine_time', 'false') == 'true'

    if args[1] == 'sync_to_server' then
        TriggerServerEvent('menu:server:updateTime', currentHour, currentMinute, timeFrozen, not timeSyncedWithMachine)
    end
    ...
    timeFrozen = GetConvar('menu_freeze_time', 'false') == 'true'
    if args[1] == 'freeze_time' then
        TriggerServerEvent('menu:server:updateTime', currentHour, currentMinute, not timeFrozen, timeSyncedWithMachine)
    elseif args[1] == 'set_time_preset' then
        TriggerServerEvent('menu:server:updateTime', args[2], 0, timeFrozen, timeSyncedWithMachine)
    elseif args[1] == 'set_time_custom' then
        if args[2] == 'hours' then
            local hour = scrollIndex - 1
            TriggerServerEvent('menu:server:updateTime', hour, currentMinute, timeFrozen, timeSyncedWithMachine)
        elseif args[2] == 'minutes' then
            local minute = scrollIndex - 1
            TriggerServerEvent('menu:server:updateTime', currentHour, minute, timeFrozen, timeSyncedWithMachine)
        end
    end
end)

lib.registerMenu({
    id = 'menu_weather_options',
    ...
    onSelected = function(selected)
        MenuIndexes['menu_weather_options'] = selected
    end,
    onCheck = function(_, checked, args)
        blackout = GetConvar('menu_enable_blackout', 'false') == 'true'
        snowEffects = GetConvar('menu_enable_snow_effects', 'false') == 'true'
        dynamicWeather = GetConvar('menu_dynamic_weather', 'true') == 'true'

        if args[1] == 'dynamic_weather' then
            checkedDynamicWeather = checked
            TriggerServerEvent('menu:server:updateWeather', currentWeather, blackout, checked, snowEffects)
            lib.setMenuOptions('menu_weather_options', {label = 'Dynamic Weather', description = 'Whether to randomize the state of the weather or not', args = {'dynamic_weather'}, checked = checked, close = false}, 1)
        elseif args[1] == 'blackout' then
            checkedBlackout = checked
            TriggerServerEvent('menu:server:updateWeather', currentWeather, checked, dynamicWeather, snowEffects)
            lib.setMenuOptions('menu_weather_options', {label = 'Blackout', description = 'If turned on, disables all light sources', args = {'blackout'}, checked = checked, close = false}, 2)
        elseif args[1] == 'snow_effects' then
            checkedSnowEffects = checked
            TriggerServerEvent('menu:server:updateWeather', currentWeather, blackout, dynamicWeather, checked)
            lib.setMenuOptions('menu_weather_options', {label = 'Snow Effects', description = 'This will force snow to appear on the ground and enable snow particles for peds and vehicles. Combine with X-MAS or Light Snow for the best results', args = {'snow_effects'}, checked = checked, close = false}, 3)
        end
    end,
    options = {
    ...
    if args[1] == 'set_weather' then
        ...
        snowEffects = GetConvar('menu_enable_snow_effects', 'false') == 'true'
        dynamicWeather = GetConvar('menu_dynamic_weather', 'true') == 'true'
        TriggerServerEvent('menu:server:updateWeather', args[2], blackout, dynamicWeather, snowEffects)
    elseif args[1] == 'remove_clouds' then
        TriggerServerEvent('menu:server:setClouds', true)
    elseif args[1] == 'randomize_clouds' then
        TriggerServerEvent('menu:server:setClouds', false)
    end
end)

RegisterNetEvent('menu:client:setClouds', function(opacity, cloudType)
...
CreateThread(function()
    while true do
        currentHour = tonumber(GetConvar('menu_current_hour', '7'))
        currentMinute = tonumber(GetConvar('menu_current_minute', '0'))
        currentHour = currentHour < 0 and 0 or currentHour > 23 and 23 or currentHour
...
CreateThread(function()
    ...
    while true do
        ...
        if checkedBlackout ~= blackout then
            lib.setMenuOptions('menu_weather_options', {label = 'Blackout', description = 'If turned on, disables all light sources', args = {'blackout'}, checked = blackout, close = false}, 2)
            checkedBlackout = blackout
            changedThings = true
        end

        if checkedSnowEffects ~= snowEffects then
            lib.setMenuOptions('menu_weather_options', {label = 'Snow Effects', description = 'This will force snow to appear on the ground and enable snow particles for peds and vehicles. Combine with X-MAS or Light Snow for the best results', args = {'snow_effects'}, checked = snowEffects, close = false}, 3)
            checkedSnowEffects = snowEffects
            changedThings = true
        end

        if checkedDynamicWeather ~= dynamicWeather then
            lib.setMenuOptions('menu_weather_options', {label = 'Dynamic Weather', description = 'Whether to randomize the state of the weather or not', args = {'dynamic_weather'}, checked = dynamicWeather, close = false}, 1)
        ...
        currentWeather = GetConvar('menu_current_weather', 'EXTRASUNNY'):upper()
        currentWeather = not weatherIndexes[currentWeather] and 'EXTRASUNNY' or currentWeather
        ...
        if currentChecked ~= currentWeather then
            ...
            lib.setMenuOptions('menu_weather_options', {label = oldData[2], args = {'set_weather', currentChecked}, close = false}, oldData[1])
            lib.setMenuOptions('menu_weather_options', {label = newData[2], icon = 'circle-check', args = {'set_weather', currentWeather}, close = false}, newData[1])
            currentChecked = currentWeather
            changedThings = true
        end

        if changedThings and lib.getOpenMenu() == 'menu_weather_options' then
            lib.hideMenu(false)
            Wait(100)
            lib.showMenu('menu_weather_options', MenuIndexes['menu_weather_options'])
            changedThings = false
        ...
        if GetNextWeatherTypeHashName() ~= joaat(currentWeather) then
            ...
            TriggerEvent('menu:client:weatherChangeComplete', currentWeather)
        end
        Wait(1000)
    end
end)